...
namespace FeedReader
{
    ...
    public class GNFeedList
    {
        public string Title { get; set; }
        public string Description { get; set; }        
        public bool IsPubDate { get; set; }
        public List<GNFeedItem> itemList { get; set; }
        public GNFeedList()
        {
            Title = "N/A";
            Description = "N/A";          
            IsPubDate = false;
            itemList = new List<GNFeedItem>();
        }
    }
    ...
    public class RSS
    {
        ...
        public static void GetXMLFeedAsList(string rssUrl, int FeedId, int FeedType)
        {
            try
            {
                ...
                string strXML = "";
                using (XmlReader myReader = XmlReader.Create(rssUrl))
                {
                    myReader.Read();                   
                ...
                if (FeedList != null)
                {
                    List<GNFeedItem> itemLists = FeedList.itemList;
                    if (itemLists != null && itemLists.Count > 0)
                    {
                        ...
                        decimal PollTime = Math.Round(Convert.ToDecimal(ts.TotalMilliseconds), 2);
                        int Counter = 0;
                        foreach (GNFeedItem row in itemLists)
                        {
                            row.FeedId = FeedId;
                            row.InsertTime = DateTime.Now;
                            if (Counter > 0)
        ...
        public static GNFeedList ParseCraiglistDocument(XmlDocument doc)
        {
            var objCraiglistFeed = new GNFeedList();
            bool IsPubDate = false;
            ...
            foreach (XmlNode node in nodeLists)
            {
                foreach (XmlNode chielNode in node.ChildNodes)
                {
                    ...
                    {
                        GNFeedItem item = new GNFeedItem();
        ...
        public static GNFeedList ParseAtomDocument(XmlDocument doc)
        {
            var objCraiglistFeed = new GNFeedList();
            bool IsPubDate = false;
        ...
        public static GNFeedList ParseXMLDocument(XmlDocument doc)
        {
            var objCraiglistFeed = new GNFeedList();
            bool IsPubDate = false;
